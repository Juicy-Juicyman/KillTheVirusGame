...
const playerOneParagraph = document.querySelector(
  ".playerOne"
) as HTMLParagraphElement;
const playerTwoParagraph = document.querySelector(
  ".playerTwo"
) as HTMLParagraphElement;

const playerOneResult = document.querySelector(
  "#playerOneResult"
) as HTMLParagraphElement;
const playerTwoResult = document.querySelector(
  "#playerTwoResult"
) as HTMLParagraphElement;

const playerOneLatestTime = document.querySelector(
  "#playerOneLatestTime"
) as HTMLParagraphElement;
const playerTwoLatestTime = document.querySelector(
  "#playerTwoLatestTime"
) as HTMLParagraphElement;

const highscoreUlEl = document.querySelector(
  "#highscoreUl"
) as HTMLUListElement;
const playedGamesUlEl = document.querySelector(
  "#recentGamesUl"
) as HTMLUListElement;
const gameOverDiv = document.querySelector("#gameOver") as HTMLDivElement;
const winnerOrLoser = document.querySelector(
  "#winnerOrLoser"
...
let checkifVirusIsShowing = false;
...
//   alert(
//     "A player disconnected from the Game, reload page to play with another player"
//   );
//   window.location.reload();
// });
...
socket.on("highscore", (allHighscores) => {
  ...
  highscoreUlEl.innerHTML = allHighscores
    .map((user) => `<li>${user.username} - ${user.averageTimeFromUser}`)
    .join("");
});

socket.on("playedGames", (allPlayedGames) => {
  console.log("All highscores", allPlayedGames);
  playedGamesUlEl.innerHTML = allPlayedGames
    .map(
      (game) =>
        `<li>${game.userOne}: ${game.userOneScore} - ${game.userTwo}: ${game.userTwoScore}`
    )
    .join("");
});
...
socket.on("gameStart", (gameroom, virusShow, virusInterval) => {
  ...
  function getDivandPutvirus(virusShow: number, virusInterval: number) {
    ...
    if (divElement && !checkifVirusIsShowing) {
      checkifVirusIsShowing = true;
      setTimeout(function () {
...
socket.on("roundWinner", (userInRoom) => {
  ...
  if (userInRoom.username === playerOneParagraph.innerText) {
    playerOneResult.innerText = `${userInRoom.score}`;
  } else {
    playerTwoResult.innerText = `${userInRoom.score}`;
  }
});

socket.on("latestReactiontime", (usersInRoom, gameroomId) => {
  if (usersInRoom[0].virusClicked) {
    let virusClickedInSeconds1: string = (
      usersInRoom[0].virusClicked / 1000
    ).toFixed(3);
  ...
  if (usersInRoom[1].virusClicked) {
    let virusClickedInSeconds2: string = (
      usersInRoom[1].virusClicked / 1000
    ).toFixed(3);
