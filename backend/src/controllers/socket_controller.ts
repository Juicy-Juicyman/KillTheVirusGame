...
export const handleConnection = (
	...
	async function findingLastPlayedGames() {
		...
		const playedGamesData: PlayedGamesUser[] = allPlayedGames.map(
			(game) => ({
				id: game.id,
	...
	socket.on(
		...
		async (
			...
			if (user1.virusClicked < user2.virusClicked) {
				...
				const user1winner = await prisma.user.update({
					where: {
						id: user1.id,
					},
					data: {
				...
				io.to(gameroom.id).emit("roundWinner", user1winner);
			} else {
				const newScore = user2.score ? user2.score + 1 : 1;
				const user2Winner = await prisma.user.update({
					where: {
						id: user2.id,
					},
					data: {
				...
				io.to(gameroom.id).emit("roundWinner", user2Winner);
			}
			// }
			...
			const endGameUsers = findFinalRoomtocompare.users.map(
				(user) => user
			);
			let userOneWithTenClick = endGameUsers[0];
			let userTwoWithTenClick = endGameUsers[1];
			if (
				userOneWithTenClick.averageTime.length !== 10 &&
				userTwoWithTenClick.averageTime.length !== 10
			) {
				io.to(gameroom.id).emit(
					"nextRound",
					gameroom,
				...
				debug("Last debug of UserOne", userOneWithTenClick);
				debug("Last debug of UserTwo", userTwoWithTenClick);
				function averageClickTime1() {
					let sum1 = 0;
					for (
						let i = 0;
						i < userOneWithTenClick.averageTime.length;
						i++
					) {
				...
				function averageClickTime2() {
					...
					for (
						let i = 0;
						i < userTwoWithTenClick.averageTime.length;
						i++
					) {
				...
				async function removeUsersandRoom() {
					...
					// Ta bort eventuella resterande användare i rummet
					...
					for (const remainingUser of remainingUsers) {
						await prisma.user.delete({
							where: {
								id: remainingUser.id,
							},
					...
					if (remainingUsers.length === 0) {
						// Alla användare i rummet har tagits bort, nu kan vi ta bort själva rummet
						...
						debug(
							"Det finns fortfarande användare kvar i rummet. Rummet kan inte tas bort."
						);
					}
				}
				removeUsersandRoom();
	...
	socket.on("disconnect", async () => {
		...
		// io.to(findUser.roomId).emit("userDisconnect");
	});
};
